;((global)=>{
		...
		for (let i = 0; i < changes.length; i++) {
			...
			for (let changekey of currentchanges) {
				...
				for (let binding of binds) {
					const handle = BIND_TYPES[binding.type].handle
					handle && handle(binding.el, this, changekey)
				}
			}
		}
	...
	CTX.prototype.add_binding = function(el, deepkey, type, selector) {
		...
		const origkey = deepkey
		if (BIND_TYPES[type].getKey)
			deepkey = BIND_TYPES[type].getKey(deepkey)

		if (!binds[deepkey])
			binds[deepkey] = []

		const event = BIND_TYPES[type].event
		if (event && this.events.indexOf(selector) === -1) {
			this.events.push(selector)
			event(selector, this, origkey)
		}

		const init = BIND_TYPES[type].init
		init && init(el, this, origkey)
	...
	// handle is for responding to ctx updates
	// event is for attaching a singleton event listener on that type
	const BIND_TYPES = {
		html: {
			handle(el, ctx, key) {
		...
		'bind-text': {
			...
			event(selector, ctx, _) {
				ctx.on('keyup', selector, function(e) {
					const key = e.target.getAttribute('data-bind-text')
		...
		'bind-checkbox': {
			...
			event(selector, ctx, _) {
				ctx.on('change', selector, function(e) {
					const key = e.target.getAttribute('data-bind-checkbox')
					const access = ctx.findParent(key)
		...
		foreach: {
			handle(el, ctx, key) { el.checked = !!ctx.find(key) },
			getKey(key) { return key.split(' in ')[1] },
			init(el, ctx, key) {
				let [itemname, arrkey] = key.split(' in ')
				el._foreacharr = arrkey
				// todo
			}
		},
		click: {
			event(selector, ctx, _) {
				ctx.on('click', selector, function(e) {
					const key = e.target.getAttribute('data-click')
					const fn = ctx.find(key)
					if (fn) return fn(e)
				})}
		}
	}

	// ...ugh arrays. I give up
